	...
	<div class="wrap">
		...
		<div class="grid">
			@foreach($images as $img)
				<div class="card">
					<div class="poster">
						<a href="{{ (Str::startsWith($img, 'http') ? $img : asset($img)) }}" target="_blank" rel="noopener">
							<img src="{{ (Str::startsWith($img, 'http') ? $img : asset($img)) }}" alt="Poster Penempatan">
						</a>
					</div>
					<div class="meta">
						...
						<div class="actions">
							<a class="btn" href="{{ (Str::startsWith($img, 'http') ? $img : asset($img)) }}" target="_blank" rel="noopener" download>Unduh</a>
							{{-- <a class="btn secondary" href="{{ url('/penempatan') }}">Lihat Semua</a> --}}
						</div>
					</div>
				</div>
			@endforeach
