    ...
    <div class="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
      ...
      <main>
        <div class="bg-white rounded-xl card-adv p-6">
          ...
          <form action="{{ route('pendaftar.akun.whatsapp.update') }}" method="POST" class="mt-5">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="md:col-span-2 field-box p-4 rounded">
                <label class="block text-sm mb-1">Nomor WhatsApp</label>
                <input type="tel" name="whatsapp" class="w-full border rounded px-3 py-2" value="{{ old('whatsapp', Auth::user()->no_hp ?? '') }}">
              </div>
            </div>
