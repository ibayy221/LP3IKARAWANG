    ...
    <div class="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
      ...
      <main>
        <div class="bg-white rounded-xl card-adv p-6">
          ...
          @endif
          @if(session('error'))
            <div class="p-3 mt-4 rounded bg-red-50 text-red-700">{{ session('error') }}</div>
          @endif
          @if($errors->any())
          ...
          <form action="{{ route('pendaftar.biodata.update') }}" method="POST" enctype="multipart/form-data" class="mt-5">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="email" name="email" class="w-full border rounded px-3 py-2" value="{{ old('email', $pendaftar->email ?? '') }}">
              </div>
              <div>
              ...
              <div>
                ...
                <select name="agama" class="w-full border rounded px-3 py-2">
                  ...
                  <option value="Lainnya" {{ (old('agama', $pendaftar->agama ?? '') == 'Lainnya') ? 'selected' : '' }}>Lainnya</option>
                </select>
              </div>
              ...
              <div>
                ...
                <input type="text" name="asal_sekolah" class="w-full border rounded px-3 py-2" value="{{ old('asal_sekolah', $pendaftar->asal_sekolah ?? '') }}">
              </div>
              <div>
                <label class="block text-sm text-slate-600 mb-1">Alamat</label>
                <input type="text" name="alamat" class="w-full border rounded px-3 py-2" value="{{ old('alamat', $pendaftar->alamat ?? '') }}">
              </div>

              <div>
                <label class="block text-sm text-slate-600 mb-1">Kecamatan</label>
                <input type="text" name="kecamatan" class="w-full border rounded px-3 py-2" value="{{ old('kecamatan', $pendaftar->kecamatan ?? '') }}">
              </div>
              <div>
                <label class="block text-sm text-slate-600 mb-1">Desa</label>
                <input type="text" name="desa" class="w-full border rounded px-3 py-2" value="{{ old('desa', $pendaftar->desa ?? '') }}">
              </div>

              <div>
                <label class="block text-sm text-slate-600 mb-1">Kode Pos</label>
                <input type="text" name="kode_pos" class="w-full border rounded px-3 py-2" value="{{ old('kode_pos', $pendaftar->kode_pos ?? '') }}">
              </div>

              <div>
                <label class="block text-sm text-slate-600 mb-1">Tahun Lulus</label>
                <input type="number" name="tahun_lulus" class="w-full border rounded px-3 py-2" value="{{ old('tahun_lulus', $pendaftar->tahun_lulus ?? '') }}">
              </div>

              <div>
                <label class="block text-sm text-slate-600 mb-1">Akun Instagram</label>
                <input type="text" name="instagram" class="w-full border rounded px-3 py-2" placeholder="@username" value="{{ old('instagram', $pendaftar->instagram ?? '') }}">
              </div>

              <div>
                <label class="block text-sm text-slate-600 mb-1">Nama Orang Tua / Wali</label>
                <input type="text" name="nama_wali" class="w-full border rounded px-3 py-2" value="{{ old('nama_wali', $pendaftar->nama_wali ?? '') }}">
              </div>

              <div>
                <label class="block text-sm text-slate-600 mb-1">Nomor Telp Orang Tua / Wali</label>
                <input type="tel" name="telp_wali" class="w-full border rounded px-3 py-2" value="{{ old('telp_wali', $pendaftar->telp_wali ?? '') }}">
              </div>
              ...
              <div class="md:col-span-2">
                ...
                @if(!empty($pendaftar->ktp_path))
                  <div class="mt-2">
                    @php $ext = strtolower(pathinfo($pendaftar->ktp_path, PATHINFO_EXTENSION)); @endphp
                    @if(in_array($ext, ['jpg','jpeg','png']))
                      <img src="{{ asset(ltrim($pendaftar->ktp_path,'/')) }}" alt="KTP" class="w-32 h-32 object-cover rounded-md border">
                    @else
                      {{-- <a href="{{ asset(ltrim($pendaftar->ktp_path,'/')) }}" target="_blank" class="text-sm text-blue-600 underline">Lihat dokumen KTP/Kartu Pelajar</a> --}}
                    @endif
                  </div>
                @endif
              </div>
              ...
              <div class="md:col-span-2">
                ...
                @if(!empty($pendaftar->ijazah_path))
                  <div class="mt-2">
                    @php $ext = strtolower(pathinfo($pendaftar->ijazah_path, PATHINFO_EXTENSION)); @endphp
                    @if(in_array($ext, ['jpg','jpeg','png']))
                      <img src="{{ asset(ltrim($pendaftar->ijazah_path,'/')) }}" alt="Ijazah" class="w-32 h-32 object-cover rounded-md border">
                    @else
                      <a href="{{ asset(ltrim($pendaftar->ijazah_path,'/')) }}" target="_blank" class="text-sm text-blue-600 underline">Lihat dokumen Ijazah</a>
                    @endif
                  </div>
                @endif
              </div>
              ...
              <div class="md:col-span-2">
                ...
                @if(!empty($pendaftar->akte_kelahiran_path))
                  <div class="mt-2">
                    @php $ext = strtolower(pathinfo($pendaftar->akte_kelahiran_path, PATHINFO_EXTENSION)); @endphp
                    @if(in_array($ext, ['jpg','jpeg','png']))
                      <img src="{{ asset(ltrim($pendaftar->akte_kelahiran_path,'/')) }}" alt="Akte Kelahiran" class="w-32 h-32 object-cover rounded-md border">
                    @else
                      <a href="{{ asset(ltrim($pendaftar->akte_kelahiran_path,'/')) }}" target="_blank" class="text-sm text-blue-600 underline">Lihat dokumen Akte Kelahiran</a>
                    @endif
                  </div>
                @endif
              </div>
              ...
              <div class="md:col-span-2">
                ...
                @if(!empty($pendaftar->surat_sudah_bekerja_path))
                  <div class="mt-2">
                    @php $ext = strtolower(pathinfo($pendaftar->surat_sudah_bekerja_path, PATHINFO_EXTENSION)); @endphp
                    @if(in_array($ext, ['jpg','jpeg','png']))
                      <img src="{{ asset(ltrim($pendaftar->surat_sudah_bekerja_path,'/')) }}" alt="Surat Bekerja" class="w-32 h-32 object-cover rounded-md border">
                    @else
                      <a href="{{ asset(ltrim($pendaftar->surat_sudah_bekerja_path,'/')) }}" target="_blank" class="text-sm text-blue-600 underline">Lihat dokumen Surat Bekerja</a>
                    @endif
                  </div>
                @endif
              </div>
